defmodule FF.Command.Build do
  ...
  defp graph_from_callback_result!(other) do
    raise ArgumentError,
          "graph callback must return a %FF.Graph{}, stream, list, keyword list, map, or nil, got: #{inspect(other)}"
  end
  ...
  defp normalize_graph_output!(name, other) do
    raise ArgumentError,
          "graph output #{inspect(name)} must be an FF.Stream, got: #{inspect(other)}"
  end
